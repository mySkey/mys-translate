(function (global, factory) {
  ...
  function MysKeyTranslate(config) {
    ...
    this.requestApi = (value, parames) => {
      ...
      const fromData = { ...parames,
        q: encodeURIComponent(q),
        sign,
        appid,
        salt
      };
      const fanyiApi = this.createUrl(this.baiduApi, fromData); // console.log("fanyiApi", fanyiApi);

      return new Promise(resolve => {
        axios.get(fanyiApi).then(({
          data: res
        }) => {
    ...
    this.translate = async (value, parames = {
      from: "zh",
      to: "en"
    }) => {
      let result = "";
    ...
    this._createObjValue = async (value, parames) => {
      ...
      const reqData = strDatas.filter(item => typeof item === "string").join("\n");
      ...
      for (let key in value) {
        ...
        if (Array.isArray(value[key]) || Object.prototype.toString.call(value[key]) === "[object Object]") {
